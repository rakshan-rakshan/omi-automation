...
function SegmentRow({ seg, showSongs, onUpdate }: SegmentRowProps) {
  ...
  const approve = async () => {
    setSaving(true);
    try {
      await approveSegment(seg.segment_id);
      onUpdate({ ...seg, review_status: 'approved' });
    } finally {
      setSaving(false);
    }
  };

  const retranslate = async (tier: Tier, modelId?: string) => {
    setTranslating(true);
    try {
      const apiKey = getStoredApiKeys().openrouter || undefined;
      const result = await translateSegment(seg.segment_id, tier, true, modelId, apiKey);
  ...
  return (
    <div
      className={clsx(
        'grid grid-cols-2 gap-0 border-b last:border-0 transition-colors',
        isSong ? 'opacity-50 bg-purple-50/30' : 'hover:bg-gray-50/60',
    ...
    >
      ...
      <div className="px-4 py-2 border-r text-sm text-gray-800 leading-relaxed font-[Noto_Sans_Telugu]">
      ...
      <div className="px-4 py-2 relative">
        ...
        {editing ? (
          <div>
            <textarea
              ...
              rows={3}
              className="w-full text-sm border rounded px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none"
            />
            <div className="flex items-center gap-2 mt-1.5">
              <button
                onClick={saveEdit}
                disabled={saving}
                className="inline-flex items-center gap-1 text-xs bg-brand-600 text-white px-3 py-1 rounded hover:bg-brand-700 disabled:opacity-50"
              >
        ...
        {!editing && (
          <div className="flex items-center gap-2 mt-1.5">
            <button
              onClick={startEdit}
              className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-brand-600 border border-gray-200 hover:border-brand-300 px-2 py-0.5 rounded"
            >
            ...
            {!isApproved && displayText && (
              <button
                onClick={approve}
                disabled={saving}
                className="inline-flex items-center gap-1 text-xs text-green-700 hover:text-green-900 border border-green-200 hover:border-green-400 px-2 py-0.5 rounded"
              >
            ...
            {!translating ? (
              <div className="flex items-center gap-1.5 ml-auto">
                <ModelSelector
                  value={null}
                  onChange={(modelId) => retranslate('good', modelId)}
                ...
                <div className="relative group">
                  <button className="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-gray-700 px-2 py-0.5 rounded border border-gray-200">
                    <RotateCcw className="w-3 h-3" /> Tier
                  </button>
                  <div className="absolute right-0 top-full mt-1 hidden group-hover:block z-10 bg-white border rounded shadow-md text-xs min-w-[100px]">
                    {(['best', 'good', 'cheap'] as Tier[]).map((tier) => (
                      <button
                        key={tier}
                        onClick={() => retranslate(tier)}
                        className="block w-full text-left px-3 py-1.5 hover:bg-gray-50 capitalize"
                      >
                        {tier}
                      </button>
                    ))}
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-4 px-4 py-2.5 border-b bg-white text-sm shrink-0">
        <span className="text-gray-500">
          {visibleCount} segments · {approvedCount} approved
        </span>
        {songCount > 0 && (
          <button
            onClick={() => setShowSongs((v) => !v)}
            className="inline-flex items-center gap-1.5 text-xs text-purple-600 hover:text-purple-800 border border-purple-200 px-2 py-0.5 rounded"
          >
